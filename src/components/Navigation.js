  ...
  return (
    <div className={classnames("flex flex-row justify-center items-center p-5", {"text-white":state.viewMode})}>
      ...
      <div className="flex flex-row text-3xl itemx-center">
        <input type="text" className={classnames("w-[50px] mr-3 border-2 rounded-xl focus:outline-none focus:border-blue-700 focus:shadow-outline",{"bg-slate-500": state.viewMode})} value={state.currentPage} onChange={(e) => {
          if(parseInt(e.target.value)>0 && parseInt(e.target.value)<= state.numberOfPage) {
            setCurrentPage(parseInt(e.target.value))
          }
        }}></input>
        <h3> of {state.numberOfPage} pages</h3>
      </div>
