...
const Navigation = () => {
  ...
  return (
    <div className={classnames("flex flex-row justify-center items-center p-5", {"text-white":state.viewMode})}>
      ...
      <div className="flex flex-row sm:text-3xl text-xl items-center">
      ...
      <div className="sm:text-4xl text-xl cursor-pointer ml-5 hover:text-blue-500"><i className="fa fa-angle-right" onClick={()=>{state.currentPage<state.numberOfPage?setCurrentPage(state.currentPage+1):(null)}}></i></div>
    </div>
  )
}
